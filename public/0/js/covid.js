am4core.ready(function(){
  ...
  $.getJSON(URL_TESTY, data => {
    /* Last Update */
    updateLastUpdate('URL_TESTY', getDateFromString(data.modified));

    tdata = data.data;
    let this_day_past_weeks = getThisDayPastWeeksData(tdata, 12);
    updateThisDayPastWeeksTests(this_day_past_weeks);
  });
  ...
  $.getJSON(URL_ZAKLADNI_PREHLED, data => {
    ...
    updateNewAndTotalCases(zak_pr.potvrzene_pripady_vcerejsi_den, zak_pr.potvrzene_pripady_celkem);
...
function updateThisDayPastWeeksTests(data) {
  data = leaveTests(data);
  for(let i = 11; i >= 1; i--){
    $(`#swt${i}`).text(addSpaces(data[i].cases));
  };
};
...
function updateThisDayPastWeeksText(day) {
  $('#thisDayPastWeeksText').replaceWith(`
    <span class='d-flex justify-content-center font-size-23 mt-5 font-weight-light'>
      New cases & tests on the past
      <span class='mx-5 d-none d-xl-block'>10</span>
      <span class='mx-5 d-none d-lg-block d-xl-none'>8</span>
      <span class='mx-5 d-none d-md-block d-lg-none'>6</span>
      <span class='mx-5 d-none d-sm-block d-md-none'>4</span>
      <span class='mx-5 d-block d-sm-none'>4</span>
      <span class='font-weight-normal'>${getDayName(day)}s</span>
    </span>
  `);
};
...
function updateNewAndTotalCases(latest_day_cases,total_cases) {
